const range = (start, amount, inc) => Array(amount).fill(inc).map((d, i) => start + ((1+i) * d))
...
const getPath = directions => {
    ...
    directionArray.forEach(d => {
        const start = path[path.length-1]
        const lrud = d.substring(0,1)
        const amount = parseInt(d.substring(1), 10)

        let line
        switch (lrud) {
            case 'R':
                line = range(start[0], amount, 1).map(p => [p, start[1]])
                break
            case 'L':
                line = range(start[0], amount, -1).map(p => [p, start[1]])
                break
            case 'U':
                line = range(start[1], amount, 1).map(p => [start[0], p])
                break
            case 'D':
                line = range(start[1], amount, -1).map(p => [start[0], p])
                break
...
const stringify = p => `${p[0]},${p[1]}`
const indexInArray = (arr, p) => arr.indexOf(stringify(p))
const pointInArray = (arr, p) => indexInArray(arr, p) > 0

const getCrossPoints = (path1, pp2) => {
    return path1.filter(p => pointInArray(pp2, p))
}

const manhattanDistanceToZero = p => Math.abs(p[0]) + Math.abs(p[1])

const calculator = (directions1, directions2) => { 
    const path1 = getPath(directions1)
    const path2 = getPath(directions2)
    const pp1 = path1.map(stringify)
    const pp2 = path2.map(stringify)

    const intersection = getCrossPoints(path1, pp2)

    const distance = intersection.reduce((acc, cp) => Math.min(acc, manhattanDistanceToZero(cp)), Number.MAX_SAFE_INTEGER)

    const wireDistance = intersection.reduce((acc, cp) => Math.min(acc, indexInArray(pp1, cp) + indexInArray(pp2, cp)), Number.MAX_SAFE_INTEGER)
...
console.log('Example', calculator('R8,U5,L5,D3', 'U7,R6,D4,L4'))
console.log('Test 1', calculator('R75,D30,R83,U83,L12,D49,R71,U7,L72', 'U62,R66,U55,R34,D71,R55,D58,R83'))
console.log('Test 2', calculator('R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51', 'U98,R91,D20,R16,D67,R40,U7,R15,U6,R7'))

console.log('Puzzle', calculator(
